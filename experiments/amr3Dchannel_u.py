...
#%% #======================= Greedy Quadratic Manifold ========================#
    ...
    np.savez("results/amr3Dchannel/u/qm_svd.npz", V=V_qm, W=W_qm, mu=mu_qm)

    #%% Or load precomputed GreedyQM results
    I_qm = np.load("results/amr3Dchannel/u/I_qm.npy")
    qm_data = np.load("results/amr3Dchannel/u/qm_svd.npz")
    V_qm = qm_data['V']
    W_qm = qm_data['W']
    mu_qm = qm_data['mu']
...
#%% #========================= Greedy Cubic Manifold ==========================#
    ...
    np.savez("results/amr3Dchannel/u/cm_svd.npz", V=V_cm, W=W_cm, mu=mu_cm)

    #%% Or load precomputed GreedyCM results
    I_cm = np.load("results/amr3Dchannel/u/I_cm.npy")
    cm_data = np.load("results/amr3Dchannel/u/cm_svd.npz")
    V_cm = cm_data['V']
    W_cm = cm_data['W']
    mu_cm = cm_data['mu']
...
#%% Load trained model 
    model_3 = torch.load(
        "results/amr3Dchannel/u/sparsemodesnet_model_pi3net.pth",
        weights_only=False,
    )
    I_nn_3 = np.load("results/amr3Dchannel/u/I_nn_pi2net.npy")
    omegas_3 = np.load("results/amr3Dchannel/u/omegas_pi2net.npy")
...
#%% %========== Configuration of SparseModesNet Pi3Net (leading-r) ============%
    ...
    config_dict = {
        ...
        'normalize_type': 'minmax',
        # Architecture
        # 'hidden_units': [200, 800, 800, 200],  # MLP
        'hidden_units': [64, 720, 624],  # PiNet
        'network_type': 'PiNetCCP',
        'poly_order': 3,
        'num_polys': 1,
...
#%% Load trained model
    model_3p = torch.load(
        "results/amr3Dchannel/u/sparsemodesnet_model_pi3net_leading.pth",
        weights_only=False,
    )
...
#%% #===================== Plot Reconstruction Errors =========================#
    ...
    #%% Or load 
    errors_data = np.load("results/amr3Dchannel/u/reconstruction_errors.npz")
...
#%% #======================= Plot reconstruction errors =======================#
    ...
    ax.set_title('Streamwise Velocity', fontsize=40)
    ax.tick_params(axis='both', which='major', labelsize=36)
    ax.grid(True, alpha=0.3)
    # ax.legend(fontsize=30, loc='lower left', frameon=False)
    ax.set_xlim(left=0)
...
#%% %============ Plot the reconstructed flow fields (heatmap) ================%
    ...
    if y_or_z_slice == 'y':
        cbar1.set_label(r'$u_1(\xi_1,\xi_3)$', fontsize=38)
    else:
        cbar1.set_label(r'$u_1(\xi_1,\xi_2)$', fontsize=38)
    ...
    if y_or_z_slice == 'y':
        plt.suptitle('Spanwise Sliced Streamwise Velocity Reconstruction Comparison', 
                     fontsize=40, y=0.935)
    else:
        plt.suptitle('Wall-Normal Sliced Streamwise Velocity Reconstruction Comparison', 
                     fontsize=40, y=0.935)
    plt.savefig(
        'figures/amr3Dchannel/u/u_recon_recomparison_' + qm_or_cm + '.pdf', 
        dpi=300, bbox_inches='tight')
    plt.show()
    plt.close(fig)
...
#%% %===================== Additional Plotting and Analysis ===================%
    ...
    if y_or_z_slice == 'y':
        plt.suptitle(
            r'Spanwise Sliced Streamwise Velocity POD Mode Reconstruction Comparison', 
            fontsize=40, y=0.935)
    else:
        plt.suptitle(
            r'Wall-Normal Sliced Streamwise Velocity POD Mode Reconstruction Comparison',
            fontsize=40, y=0.935)
    plt.savefig(
        'figures/amr3Dchannel/u/u_spatial_modes_comparison.pdf', 
        dpi=300, bbox_inches='tight'
    )
    plt.show()
    plt.close(fig)
