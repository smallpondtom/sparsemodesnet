...
if __name__ == "__main__":
    ...
    hidden_units_burg = [128, 32896]
    hidden_units_burg = [30, 465, 900]

    # Parameter‐grid for CV
    lambdas_cv = np.logspace(-6, -2, 10)
    ...
    # ---------- Burgers' Equation ----------
    X_burgers, xspan_b, tspan_b = generate_burgers_data(
        nx=n_grids, nt=1000, nu=0.01, x_max=1.0, t_max=1.0)
    ...
    # # normalize the data 
    # mu    = X_burgers.mean(axis=1, keepdims=True)         # (d,1)
    # sigma = X_burgers.std(axis=1, keepdims=True) + 1e-6   # (d,1)
    # X_burgers = normalize(X_burgers, mu, sigma)  # (d,n) 

    #%%
    model_burg, info_burg, selected_b, freq_tab = run_sparsemodesnet(
        X_np            = X_burgers,
        s               = s_b,
        ...
        r_max           = 15,           # max modes for constraint stopping
    ...
    U_s20, _, _ = compute_pod_basis(X_burgers, s=s_b)
    fig, axes = plt.subplots(4, 5, figsize=(15, 8))
    for i, ax in enumerate(axes.flatten()):
        ax.plot(xspan_b, U_s20[:, i])
        ax.set_title(f"Mode {i+1}")
        ax.grid(True)

    plt.tight_layout()
    plt.savefig('../figures/burgers_pod_modes.png', dpi=300)
    plt.show()
    
    #%% Plot the POD modes vs the reconstruction error 
    U, S, _ = np.linalg.svd(X_burgers, full_matrices=False)
    Us_20 = U[:, :s_b].astype(np.float64)  # First s_h POD modes
    fig, ax = plt.subplots(figsize=(8, 6))
    proj_err = []
    X_heat_f64 = X_burgers.astype(np.float64)
    ...
    if reg_path == 'dense2sparse':
        ...
        plt.tight_layout()
        plt.savefig('../figures/burgers_path_summary.png', dpi=300)
        plt.show()
        plt.close(fig)
    ...
    if reg_path == 'dense2sparse':
        ...
        plt.tight_layout()
        plt.savefig('../figures/burgers_lcurve.png', dpi=300)
        plt.show()
        plt.close(fig)
    ...
    if reg_path == 'dense2sparse':
        ...
        plt.tight_layout()
        plt.savefig('../figures/burgers_lambda_analysis.png', dpi=300)
        plt.show()
        plt.close(fig)
        

    #%% Plot the reconstructed flow fields (heatmap)
    V, _, _ = np.linalg.svd(X_burgers, full_matrices=False)
    V_selected = V[:, selected_b]
    fig, ax = plt.subplots(figsize=(12, 6))
    X_pod_recon = V_selected @ V_selected.T @ X_burgers
    
    # Fix: Convert numpy array to tensor and move to correct device
    Z_input = torch.from_numpy(
        (V[:, :s_b].T @ X_burgers).T.astype(np.float32)).to(device)
    with torch.no_grad():
        model_burg.eval()
        _, X_sparse_recon_tensor = model_burg(Z_input)
        X_sparse_recon = X_sparse_recon_tensor.cpu().numpy().T 
    
    # Calculate errors
    pod_error = X_burgers - X_pod_recon
    sparse_error = X_burgers - X_sparse_recon
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    # (1,1) POD reconstruction
    im1 = axes[0,0].imshow(
        X_pod_recon, aspect='auto', cmap='viridis', origin='lower',
        extent=[tspan_b[0], tspan_b[-1], xspan_b[0], xspan_b[-1]])
    ...
    with torch.no_grad():
        model_burg.eval()
        _, X_test_recon_tensor = model_burg(Z_test)
    ...
    plt.tight_layout()
    plt.savefig('../figures/burgers_test_results.png', dpi=300)
    plt.show()
    plt.close(fig)
# %%
