...
class QuadraticManifold(nn.Module):
    def __init__(self, pod_basis: torch.Tensor, 
        ...
        self.first_layer = MaskedLayer(
            self.s, self.s, torch.eye(self.s, dtype=torch.float32), 
            dtype=torch.float32)
...
def lstsq_l2_numpy(A, B, reg_magnitude=1e-6):
    ...
    if B.ndim == 1:
        ...
        # B is 2D: shape (m, k)
        UTB = U.T @ B  # shape (min(m,n), k)
        x = Vt.T @ (sinv[:, np.newaxis] * UTB)  # shape (n, k)
...
#%% #============== Generate the Modes while Adding Dummy Modes ===============#
    print("\n" + "="*60)
    print("GENERATE MODES WITH DUMMIES")
    print("="*60)

    n_dummy = 40
    n_mode = s - n_dummy
    V_mode = np.linalg.svd(X, full_matrices=False)[0][:, :n_mode]
    V_dummy = np.random.randn(d, n_dummy)
    V = np.hstack((V_mode, V_dummy))  # (d, s)
    # Shuffle the columns of V
    shuffle_indices = np.random.permutation(s)
    V = V[:, shuffle_indices]  # (d, s)
    I_nn = np.where(shuffle_indices < n_mode)[0]
    print(f"V shape: {V.shape}")
    print(f"Number of modes: {n_mode}, Number of dummy modes: {n_dummy}")

    # Generate the optimal quadratic weight matrix
    p = n_mode*(n_mode+1) // 2  
    residual = X - V_mode @ (V_mode.T @ X) 
    Z_quad = quadratic_mapping_numpy(X.T @ V_mode)
    W = lstsq_l2_numpy(Z_quad, residual.T)[0].T
    print(f"W shape: {W.shape}")
    print(f"W dtype: {W.dtype}")

    # Generate new data with quadratic combinations of modes included
    X = V_mode @ (V_mode.T @ X) + W @ quadratic_mapping_numpy(X.T @ V_mode).T
...
#%% #======================= LassoNet Mode Selection ==========================#
    ...
    # Prep the data
    ds_sub = PODReconDataset(Z_np=Z_np, X_np=X, type="float32")
    dl_sub = DataLoader(ds_sub, batch_size=400, shuffle=True)
...
# %% #==================== Compute Reconstruction Errors ======================#
    ...
    recon_answer = np.linalg.norm(
        X - V_mode @ V_mode.T @ X - W @ quadratic_mapping_numpy(X.T @ V_mode).T, 
        'fro'
    )
    recon_answer_rel = recon_answer / np.linalg.norm(X, 'fro')
    residual = X - V_nn @ V_nn.T @ X
    W_nn, _ = lstsq_l2_numpy(
        quadratic_mapping_numpy(X.T @ V_nn), 
        residual.T, 
    )
    recon_lassonet = np.linalg.norm(
        X - V_nn @ V_nn.T @ X - W_nn.T @ quadratic_mapping_numpy(X.T @ V_nn).T, 
        'fro'
    )
...
# %% #========================= Plot the omegas ===============================#
    ...
    plt.tight_layout()
    plt.savefig('figures/lassonet/quad/noisy/omega_evolution.png', dpi=200)
    plt.show()
...
true_positives = len(np.intersect1d(I_guess, I_nn))
false_positives = len([m for m in I_nn if m not in I_guess])
false_negatives = len([m for m in I_guess if m not in I_nn])
...
foo = np.sqrt(len(I_count)) 
if foo != int(foo):
    foo2 = 10
    foo1 = len(I_count) // foo2
else:
    foo1 = int(foo)
    foo2 = foo1
count_matrix = I_count.reshape(foo1, foo2)
im4 = ax4.imshow(count_matrix, cmap='viridis', alpha=0.8)
ax4.set_title(f'Mode Selection Frequency ({foo1}×{foo2} Grid)', fontsize=14, fontweight='bold')
...
colors_rank = ['green' if mode in I_guess else 'orange' if mode in I_nn else 'gray' 
...
plt.savefig('figures/lassonet/quad/noisy/comprehensive_analysis.png', dpi=300, bbox_inches='tight')
plt.show()
...
print(f"  Selected:     {sorted(I_nn)}")
print(f"  Missed:       {sorted([m for m in I_guess if m not in I_nn])}")
print(f"  False picks:  {sorted([m for m in I_nn if m not in I_guess])}")
...
plt.tight_layout()
plt.savefig('figures/lassonet/quad/noisy/singular_values.png', dpi=200, bbox_inches='tight')
plt.show()
