...
#%%
runs = 50
thetas = torch.randn([s,runs], device=device)
# Warm‐up
for i in range(10):
    theta = thetas[:, i]
    i = proximal_old(theta, W1_T)
...
t1 = time.time()
print(f"Old loop+prox: {(t1 - t0)/runs:.4f} sec per pass")
...
for i in range(runs):
    ...
    print(f"b: {u[:5].tolist()}, W: {v[:5, :].tolist()}")  # Print first 5 for brevity
t3 = time.time()
print(f"Batched prox: {(t3 - t2)/runs:.4f} sec per pass")
# %%
# Compare prox vs proximal_batched accuracy
N = 1000
s, K = 400, 128
max_error_b = 0.0
max_error_W = 0.0
...
print("Comparing prox vs proximal_batched accuracy...")

for i in range(N):
    # Generate random theta for this iteration
    theta_test = torch.randn(s, device=device)
    W1_T_test = torch.randn(s, K, device=device)
    
    # Get results from both methods
    b_old, W_old = proximal_old(theta_test.clone(), W1_T_test.clone())
    b_batch, W_batch = proximal_batched(theta_test.clone(), W1_T_test.clone())
    
    # Compute errors
    error_b = torch.max(torch.abs(b_old - b_batch)).item()
    error_W = torch.max(torch.abs(W_old - W_batch)).item()
    
    # Track max and average errors
    max_error_b = max(max_error_b, error_b)
    max_error_W = max(max_error_W, error_W)
    avg_error_b += error_b
    avg_error_W += error_W
    
    print(f"Iteration {i+1}: b_error = {error_b:.2e}, W_error = {error_W:.2e}")

avg_error_b /= N
avg_error_W /= N

print(f"\nSummary over {N} iterations:")
print(f"Max b error: {max_error_b:.2e}")
print(f"Max W error: {max_error_W:.2e}")
print(f"Avg b error: {avg_error_b:.2e}")
print(f"Avg W error: {avg_error_W:.2e}")

# Check if they're approximately equal
tolerance = 1e-10
if max_error_b < tolerance and max_error_W < tolerance:
    print(f"\n✓ Methods are equivalent within tolerance {tolerance}")
else:
    print(f"\n✗ Methods differ beyond tolerance {tolerance}")
# %%
