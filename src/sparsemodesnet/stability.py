...
def run_sparsemodesnet_ss(X_np: np.ndarray, config: SparseModesNetConfig):
    """
    ...
    m = len(config.selection.lambdas)
    counts = np.zeros((config.s, m), dtype=int)

    # 1) subsample loop
    for i, lam in enumerate(config.selection.lambdas):
        print(f" SS testing λ = {lam:.3e}. Currently {i+1}/{m} ...")
        for _ in range(config.selection.num_subsamples):
            subsamp = np.random.choice(n, size=n//2, replace=False)
            ds_sub = PODReconDataset(Z_np=Z_np[:, subsamp], X_np=X_np[:, subsamp])
            dl_sub = DataLoader(ds_sub, batch_size=config.training.batch_size, shuffle=True)

            model = SparseModesNet(
                pod_basis     = torch.from_numpy(U_s_np).to(config.training.device),
                input_dim     = config.s,
            ...
            train_sparsemodesnet(
                model, dl_sub, config.training.num_epochs, config.training.lr, 
                config.training.optimizer, config.training.device)

            omega_opt = model.omega.detach().cpu().numpy()
            counts[:, i] += (np.abs(omega_opt) > config.sparsity.nonzero_thresh).astype(int)

        # This is only for logging
        freqs_i = counts[:, i] / float(config.selection.num_subsamples)
        stable_count_i = int((freqs_i >= config.selection.pi_thresh).sum())
        print(f"  → λ = {lam:.3e} | stable features = {stable_count_i}", 
              f" (freq ≥ {config.selection.pi_thresh})")
        
        if stable_count_i == config.s:
    ...
    freqs = counts / float(config.selection.num_subsamples)  # (s, m)
    pi_max = freqs.max(axis=1)  # (s,)
    
    # 3) compute stable set
    S_stable = np.where(pi_max >= config.selection.pi_thresh)[0]
...
#             model = SparseModesNet(
#                 pod_basis     = torch.from_numpy(U_s_np).to(device),
#                 input_dim     = s,
