...
def run_sparsemodesnet_cv(X_np: np.ndarray, config: SparseModesNetConfig):
    """
    ...
    folds = np.array_split(indices, config.selection.k_folds)

    path_history_cv = []
    total_lambdas = len(config.selection.lambdas)
    selected_indices = np.zeros((config.s, total_lambdas))
    ct = 1

    for index, lam in enumerate(config.selection.lambdas):
        val_errors = []
        r_folds = []
        print(f" CV testing λ = {lam:.3e}. Currently {ct}/{total_lambdas} ...")
        for fold_idx in range(config.selection.k_folds):
            # train indices = all except this fold
            val_idx = folds[fold_idx]
            train_idx = np.hstack(
                [folds[i] for i in range(config.selection.k_folds) if i != fold_idx])
            ...
            dl_train = DataLoader(
                ds_train, batch_size=config.training.batch_size, shuffle=True, drop_last=False
            )
            dl_val = DataLoader(
                ds_val, batch_size=config.training.batch_size, shuffle=False, drop_last=False
            )

            # Instantiate a fresh model at λ
            model_cv = SparseModesNet(
                pod_basis      = torch.from_numpy(U_s_np.astype(np.float32)).to(config.training.device),
                input_dim      = config.s,
            ...
            train_sparsemodesnet(
                model_cv, dl_train, config.training.num_epochs, config.training.lr, 
                config.training.optimizer, config.training.device)
            ...
            with torch.no_grad():
                for z_b, x_b in dl_val:
                    z_b = z_b.to(config.training.device)
                    x_b = x_b.to(config.training.device)
                ...
                r_fold = int((np.abs(omega) > config.sparsity.nonzero_thresh).sum())
                r_folds.append(r_fold)
                S_stable = np.where(omega >= config.sparsity.nonzero_thresh)[0]
                selected_indices[S_stable, index] += 1 / config.selection.k_folds
    ...
    weighted_selection_freq = np.zeros(config.s)
    ...
    print(f"  Selected {len(I_CV)} modes out of {config.s}")
